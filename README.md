...
- Первый слой планшетного renderer расширен: LAS units выводятся в шапке треков, порядок параметров берется из выбора пользователя, цвета треков настраиваются, маркеры и ручные интерпретационные зоны/интервалы включаются в печатный HTML-отчет. В плане остаются mud-gas reference-треки, расширенные режимы заливки и дальнейшее развитие полного инженерного отчета.
