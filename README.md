...
- Интерпретационные depth-графики с ручным диапазоном глубины, ручным X-масштабом, режимом `Планшет` для любых числовых параметров, LAS units в шапках треков, индивидуальными цветами треков, маркерами глубины и HTML-выгрузкой для печати с таблицей маркеров.
...
- Сохранение версий экспортов проекта: HTML-отчеты и CSV выбранных интервалов с последующим скачиванием из проекта.
...
- Первый слой планшетного renderer расширен: LAS units выводятся в шапке треков, порядок параметров берется из выбора пользователя, цвета треков настраиваются, а таблица маркеров включается в печатный HTML-отчет. В плане остаются редактируемые зоны/интервалы и дальнейшее развитие полного инженерного отчета.
