...
- Поддержка разных названий кривых: `Depth`, `DEPT`, `MD`, `CH4`, `Methane`, `i-C4`, `n-C4`, русские названия и другие алиасы; Curve Manager для rename, alias, grouping, categories, units manager, metadata editor и merge LAS-кривых.
...
- Глобальная командная палитра `Ctrl+K / поиск по проекту` ищет разделы, рабочие действия, проекты, скважины, LAS-файлы, кривые, расчеты, отчеты и документацию. Добавлены категории, недавние команды, избранное, ранжирование результатов и кнопка `Открыть`, которая переключает пользователя в нужный рабочий раздел.
...
- Quick Actions Wiring делает быстрый доступ на Dashboard реальным рабочим навигатором: проект, импорт, LAS-редактор, корреляция, графики/отчеты, инструкции, настройки и лицензия создаются из единого реестра, имеют tooltip-подсказки и переключают активный раздел приложения.
- Responsive Dashboard Layout добавляет явные breakpoints для 1366×768, 1440×900, 1600×900, 1920×1080, 2K/ultrawide/4K, tablet и mobile: Dashboard перестраивается из трех колонок в две и одну, скрывает низкоприоритетные виджеты на узких экранах и запрещает горизонтальный overflow.
