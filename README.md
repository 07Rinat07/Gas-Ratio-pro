...
Карточка скважины хранит metadata проекта: рабочее название, статус, комментарий, координаты X/Y, широту/долготу, отметку KB и отметку GL в метрах. Географические координаты, KB и GL валидируются перед сохранением; LAS-файлы и расчетные snapshots при этом не читаются и не копируются.

## Well Manager metadata

Карточка скважины в активном проекте хранит только metadata в `well_cards.json`: название, статус, комментарий, координаты, KB, GL, плановую/фактическую TD и дату начала бурения `spud_date` в формате `YYYY-MM-DD`. Эти поля отображаются в Project Explorer и не изменяют сохраненные LAS-версии или расчетные snapshots.
