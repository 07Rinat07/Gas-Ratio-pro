...
- Локальные проекты в `data/projects/<project_id>/`: карточка `project.json`, карточки скважин, версии исходных и подготовленных LAS проекта, открытие сохраненных LAS без повторной загрузки, расчетные snapshots с mapping/предупреждениями/CSV/XLSX, карточкой выбранного расчета с режимом `Ch`, кратким mapping и предупреждениями перед открытием snapshot в графиках, быстрым фильтром по источнику, предупреждениям и колонкам, безопасным сравнением двух snapshots и CSV/HTML-выгрузкой результата сравнения, настройки интерпретационных графиков, сохраненные версии экспортов и HTML-отчетов, ZIP-выгрузка выбранных версий в `LAS`, `XLSX`, `CSV`, архивирование ошибочно сохраненных версий и настройки LAS-корреляции `correlation_settings.json`.
...
- Сохранение расчетных snapshots проекта: расчетная таблица, mapping, режим `Ch`, предупреждения, выгрузки `CSV`/`XLSX` и экспорт сравнения двух snapshots в `CSV`/`HTML`.
