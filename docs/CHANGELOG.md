...
- Зафиксированы минимальные физические размеры шрифтов, линий и дорожек.
- Широкие инженерные планшеты автоматически делятся на страницы с сохранением порядка треков, данных и шкал.
- PDF и SVG используют полный список страниц, page clip и корректный source-offset transform; глубинные метки повторяются на следующих страницах.
- Добавлен PNG renderer, который растеризует те же SVG-страницы без повторного расчёта геометрии.
- HTML/PDF/DOCX отчёты и asset bundles сохраняют все страницы визуализации.
- Geometry signature и export QA обновлены для агрегированной многостраничной проверки.
- Добавлены регрессии A4/A3 portrait/landscape, физической типографики и совпадения PDF/SVG/PNG page count.
...
- Перенесены легенды экранных depth-графиков под область построения, чтобы они не перекрывали заголовки.
- Для интерпретационного планшета скрыта дублирующая общая легенда: кривые уже идентифицируются заголовками дорожек.
- Заголовок планшета и заголовки дорожек разведены по отдельным вертикальным уровням.
- Убрано наложение заключения Pixler и бейджа вероятного флюида на заголовок.
- Сигнатура экспорта теперь включает BUILD_VERSION, поэтому после обновления не возвращаются устаревшие PDF/DOCX из кэша.

## v222.62 — Well Plot Collision & Print Scale Hardening
- Разделены заголовок графика и легенда на экране.
- Детальные планшеты печатаются группами не более пяти кривых на страницу.
- Увеличены печатная типографика, экспортное разрешение и полезная площадь изображения.
- Встроенная легенда планшета отключена; расшифровка остаётся в таблицах отчёта.
## v222.60 — Track Layout Engine Foundation

- Добавлены семантические ширины дорожек вместо одинаковых колонок: интерпретация, C1 и нефтяной индикатор получают больше пространства.
- Введены отдельные профили компоновки `screen` и `print`.
- Добавлен автоматический размер заголовков дорожек в зависимости от плотности планшета.
- Сокращённые подписи сохраняются без пересечений; технические идентификаторы не выводятся пользователю.
- Добавлены регрессионные тесты Track Layout Engine.
...
### Added
- Модуль `core/reservoir_ranking.py`.
- Рейтинг интервалов с прозрачным разложением балла и явными штрафами.
- Интерактивная таблица рейтинга в Data Workspace и Interpretation.
- Таблица приоритетных интервалов в PDF/DOCX.

### Changed
- Выбор строки рейтинга обновляет общий выбранный пласт во всех связанных представлениях.

## Reservoir Ranking 2.0 — настраиваемые профили

- Добавлены встроенные профили ранжирования: стандартный, нефтяной, газовый, геологоразведочный и эксплуатационный.
- Инженер может изменять веса достоверности, согласованности методик, полноты C1–C5 и мощности; веса автоматически нормализуются до 100%.
...
- Добавлена ранняя валидация параметров Professional Export.
- Добавлена защита от повторного параллельного запуска формата.
- Введены ограниченные LRU-кэши моделей и готовых файлов.
- Добавлена проверка непустого содержимого экспортного файла.
...
## v222-rc6 — Engineering Workspace Refactoring

- добавлен ограниченный LRU-кэш Plotly-наборов для нескольких применённых представлений;
- переключение между ранее построенными настройками не требует повторной сборки всех фигур;
- интерактивные графики получили стабильные Streamlit keys для повторного использования frontend-компонентов;
...
- Исправлена нормализация имени DOCX-артефакта в едином ExportController.
- Планшет переведён к единому полотну с общей легендой и одной шкалой глубины.
- Добавлено автосмасштабирование треков, усилено выделение приоритетного интервала.
- В подписи интервалов добавлены границы глубины, тип флюида и достоверность.
...
## v222.13 — Multi-Interval Report Atlas
- Добавлен обзорный планшет всей скважины и автоматические детальные планшеты по группам интервалов.
- Соседние нефтяные, газовые, газоконденсатные и иные продуктивные интервалы объединяются в одну детальную страницу.
- Для каждого детального планшета рассчитывается адаптивный запас глубины и выводится карточка интервалов.
- Клиентский профиль ограничен пятью наиболее значимыми группами; инженерный — пятнадцатью, экспертный — тридцатью.
- Детальные страницы формируются в порядке глубины и получают отдельные разрывы страниц в PDF/DOCX.


## v222.14 — Export Discoverability and Operation Status
- Панель печати перенесена перед экранными графиками и раскрыта по умолчанию.
- Добавлен крупный CTA для подготовки файла и отдельная заметная кнопка скачивания.
- Добавлен режим «Вся скважина и все УВ-интервалы», который передаёт в отчёт полный LAS и активирует обзорный планшет с детальным атласом.
...
## Interpretation interval type reassignment

- Added project-wide reassignment of intervals from one catalog type to another.
...
## v222.49 — README format coverage and documentation currency
...
## v222.59 — Plot Typography & Readability Hardening
- Увеличены шрифты заголовков треков, осей, делений и аннотаций в PDF/DOCX.
- Дублирующая легенда внутри печатного изображения отключена: обозначения остаются в отдельной читаемой таблице.
