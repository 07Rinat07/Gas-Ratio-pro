...
Важно: текущий режим `Ch` в приложении пока отличается от формулы Haworth из
этого источника и помечен как требующий подтверждения. Перед изменением расчета
нужно сделать отдельный migration/test step и сравнить результаты на контрольных
данных.
...
поддерживает авто/ручной X-scale по каждому параметру, глубину вниз, маркеры и
таблицу маркеров. Следующие доработки: units из LAS в шапке, индивидуальный цвет
и порядок треков, редактируемые зоны и печатный отчет с таблицей интерпретации.
