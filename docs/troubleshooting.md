...
Если сообщение появилось в истории, но ответа пока нет, проверьте режим ответа.
`Быстро: база знаний` должен отвечать почти сразу. `Ollama: локальная модель`
может готовить первый ответ 20-120 секунд. В `logs/app.log` это видно как
