...
В приложение добавлена отдельная вкладка `Лицензия`. Быстрый доступ `Лицензия`, командная палитра и верхняя навигация теперь ведут на самостоятельную страницу, а не на блок внутри Dashboard. Страница показывает фирменный license header, статус proprietary-лицензии, владельца, copyright, контакт автора, краткие карточки правил использования, EULA placeholder и полный текст файла `LICENSE` внутри читаемой панели.
...
Текущий следующий незавершенный пункт: **Licensing and Commercial Protection → EULA document**.
