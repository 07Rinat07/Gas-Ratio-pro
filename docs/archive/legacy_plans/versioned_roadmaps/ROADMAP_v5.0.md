...
## Phase 8.1 — Hydrocarbon Interval Interpretation and Printable Reports

Этот блок является ключевой прикладной целью GAS RATIO PRO: приложение должно находить интервалы с признаками углеводородов, объяснять интерпретацию, строить графики с маркерами и формировать нормальный печатный отчет без технической свалки.

Обязательные функции:

- Hydrocarbon Interval Detector;
- автоматическое выделение кровли, подошвы, толщины и типа признака;
- Interpretation Engine с понятным инженерным текстом по каждому интервалу;
- графики LAS с цветными маркерами интервалов;
- таблица интервалов для печати;
- PDF/XLSX/DOCX отчет;
- связь интервалов с петрофизикой: porosity, saturation, shale volume, net pay, cutoff;
- подготовка результатов для корреляции, моделирования и volumetrics;
- toolbar/ribbon кнопки: Detect HC Intervals, Interpret, Plot with Markers, Create Report, Export, Print.

Подробный план: `docs/15_Implementation_Plan/HYDROCARBON_INTERVAL_REPORTING_PLAN.md`.

Acceptance Criteria:

1. Интервалы с признаками УВ выделяются автоматически.
2. Каждый интервал имеет top, base, thickness, evidence и confidence.
3. Для каждого интервала формируется текстовая интерпретация.
4. Графики показывают зоны интереса маркерами и цветными интервалами.
5. Печатный отчет содержит сводку, таблицу интервалов, графики и выводы.
6. Отчет не содержит сырой технической свалки и служебных логов.
...
Sprint 1.5 is a functional freeze. No new engineering modules are added until the platform starts, compiles, deletes, indexes and refreshes reliably.

Current stabilization focus:

- Storage Lifecycle Framework integration.
- Service compatibility contracts.
