...
## Phase 3.1 — Application Refactoring v5

Цель этапа — убрать зависимость бизнес-логики от прямого хаотичного изменения `st.session_state` и исправить проблему, когда старые проекты, скважины, таблицы, статистики и графики возвращаются после перезапуска приложения.

Обязательные изменения:

1. **Application State Controller**
   - единая точка управления активным проектом, скважиной, LAS и workspace;
   - отдельные ключи для UI-виджетов и отдельные ключи для бизнес-контекста;
   - запрет прямого изменения widget-owned state после создания Streamlit-виджетов;
   - pending-переключение проекта после создания нового проекта.

2. **Project Persistence Cleanup**
   - удаление проекта должно удалять каталог проекта из `data/projects`;
   - удаление не должно ограничиваться очисткой `session_state`;
   - после удаления активный проект переключается на default;
   - таблицы, расчеты, графики и статистики очищаются.

3. **Well Persistence Cleanup**
   - удаление скважины должно удалять каталог скважины из `data/wells`;
   - удаление версии скважины должно удалять каталог версии и обновлять manifest;
   - после удаления очищаются таблицы редактора, интерпретационные графики, корреляция и статистика.

4. **LAS Editor Table Refactoring**
   - таблицы должны поддерживать редактирование ячеек;
   - таблицы должны поддерживать добавление строк;
   - таблицы должны поддерживать удаление строк;
   - сохранение должно записывать актуальное состояние в persistent storage или рабочую копию;
   - удаленные данные не должны восстанавливаться после перезапуска.

5. **UI Integration**
   - создание проекта не должно вызывать `StreamlitAPIException` по ключу `active_project_id`;
   - кнопка `Создать LAS` должна быть подключена к реальному экрану, а не только к backend manifest;
   - все очистки должны логироваться.

Реализация:

- `core.application_state`;
- `core.session_state_manager`;
- `projects.repository.delete_project`;
- `wells.repository.delete_well_record`;
- `wells.repository.delete_well_version`;
- `app.streamlit_app` project selector refactoring;
- `app.streamlit_app` well deletion UI.

Acceptance Criteria:

1. Новый проект создается без `StreamlitAPIException`.
2. Переключение проекта очищает старые таблицы, статистику, графики и временные данные.
3. Удаленный проект не появляется после перезагрузки.
4. Удаленная скважина не появляется после перезагрузки.
5. Удаленная версия скважины удаляется из manifest и с диска.
6. User settings, theme, license и EULA не очищаются.
