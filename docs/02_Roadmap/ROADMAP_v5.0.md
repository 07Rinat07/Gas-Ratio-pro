...
## Phase 2.2 — Workspace Data Management

Обязательный этап Roadmap v5.0 для устранения зависших таблиц, архивов и старых записей в интерфейсе.

Каждая таблица, архив и список сохраненных объектов должны иметь единый набор действий:

- обновить таблицу;
- удалить выбранные записи;
- очистить всю таблицу;
- физически удалить связанные файлы с диска;
- очистить зависимые session-state ключи;
- выполнить `st.rerun()` после изменения persistent storage;
- показывать подтверждение для массовых операций.

Первыми подключаются:

1. **Сохраненные экспорты проекта**
   - удаление выбранных экспортов;
   - очистка всего архива экспортов;
   - удаление папок `data/projects/<project_id>/exports/<export_id>`;
   - сброс `exports.json`.

2. **LAS-файлы проекта**
   - удаление выбранной LAS-версии;
   - очистка всех LAS-версий проекта;
   - удаление папок `data/projects/<project_id>/las_files/<las_file_id>`;
   - сброс LAS manifest.

3. **Следующие таблицы для подключения к единому toolbar**
   - расчеты проекта;
   - сохраненные графики;
...
1. После очистки таблица становится пустой без ручного удаления файлов.
2. После перезагрузки страницы удаленные записи не появляются снова.
3. Удаление строки удаляет не только запись manifest, но и связанную папку/файл.
4. Очистка таблицы не затрагивает исходные LAS пользователя вне рабочей копии проекта.
5. Все массовые операции требуют подтверждения.
