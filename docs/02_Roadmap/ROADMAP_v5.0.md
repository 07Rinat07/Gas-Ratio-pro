...
### Service Compatibility Pass — WellManagerService

- Well deletion is routed through `DeleteEngine`.
- Well version deletion is routed through `DeleteEngine`.
- Registered well preview file handles, resources and cache entries are released before destructive operations.
- Deleting the last version deletes the complete well folder so empty wells cannot reappear after restart.
- Compatibility aliases are preserved for the current Streamlit UI during Sprint 1/Sprint 1.5.
- Added service contract document: `docs/09_Architecture/contracts/WELL_MANAGER_SERVICE_CONTRACT.md`.
