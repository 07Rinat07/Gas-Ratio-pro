...
## B.11 LAS Quality Control Professional Foundation

Модуль `las_editor.las_quality_control` добавляет отдельный слой контроля качества LAS-таблиц и каротажных кривых. В отличие от структурного валидатора LAS, который проверяет секции и согласованность `~Curve`/`~ASCII`, Quality Control анализирует инженерное качество самих данных.

### Назначение

LAS Quality Control предназначен для предварительной диагностики данных перед расчетами, интерпретацией, импортом в моделирование и экспортом в новый LAS-файл.

### Поддерживаемые проверки

- дубликаты глубины;
- немонотонная глубина;
- отсутствующие интервалы глубины;
- нерегулярный шаг глубины;
- пустые и NULL-значения;
- отрицательные значения для кривых, где они недопустимы;
- выход значений за ожидаемый инженерный диапазон;
- резкие скачки кривых;
- flat-line участки;
- статистические выбросы;
- предупреждения по несоответствию единиц измерения.

### Встроенные профили качества

Foundation-версия содержит профили для типовых кривых: `GR`, `CALI`, `RHOB`, `NPHI`, `POR`, `SW`, `SO`, `SG`, `PERM`, `RT`, `C1`, `C2`, `C3`, `C4`, `C5`.

### Результаты работы

- `LasQualityReport`;
- список `LasQualityIssue`;
- UI-ready таблица замечаний;
- UI-ready таблица профилей качества;
- manifest проверки;
- Markdown-отчет.

### Критерии готовности

- проверка не изменяет исходную LAS-таблицу;
- ошибки и предупреждения возвращаются в структурированном виде;
...
## B.13 Mud Gas Interpretation Toolkit Foundation

Подсистема предназначена для интерпретации кривых бурового газа `C1`, `C2`, `C3`, `C4`, `C5` и построения инженерной оценки флюида по глубине.

Реализуемые сущности:

- `MudGasRatioSet` — набор расчетных коэффициентов для одной точки глубины;
- `MudGasInterpretationRow` — интерпретация одной точки глубины;
- `MudGasIntervalSummary` — агрегированная интерпретация интервала;
- `MudGasInterpretationResult` — общий результат интерпретации;
- `MudGasIssue` — диагностическое сообщение.

Поддерживаемые методы:

- Haworth Wetness Ratio;
- Haworth Balance Ratio;
- Haworth Character Ratio;
- Pixler C1/C2;
- Oil Indicator;
...
- исходный LAS не изменяется;
- интерпретация выполняется в памяти;
