...
Статус: завершено. Реализовано разделение инженерного и клиентского отчета, сокращены повторяющиеся таблицы и метаданные, унифицированы заключения и ограничения, очищены PDF/DOCX от служебных полей.
...
- ExportRequest валидируется до построения модели и запуска renderer.
- Добавлена защита от повторного запуска одного и того же экспортного запроса.
- Кэши модели и артефактов переведены на ограниченный LRU-контракт.
- Пустой результат renderer считается контролируемой ошибкой.
- Добавлена точечная очистка экспортного кэша проекта.
- README.md не изменялся.


## v222-rc4 — Isolated Professional Export panel

- Панель Professional Export вынесена в отдельный `st.fragment`.
- Изменение профиля, формата и диапазона печати больше не перезапускает всю страницу интерпретации.
...
Engineering Workspace Refactoring реализован: Plot Cache 2.0 ограничен четырьмя наборами фигур, планшет получил адаптивные ширины инженерных дорожек и изолированные заголовки. Следующая проверка — реальный LAS и ручная оценка скорости переключения представлений.

## v222-rc7 — Engineering Core Refactoring

Статус: реализовано и проверено.

- Изолированы пространства состояния для графиков, экспорта и рабочих модулей.
- Добавлена ограниченная runtime-диагностика длительности этапов, статуса кэша, renderer и оценки памяти.
- Plot Cache публикует hits, misses, evictions, entries и estimated_bytes.
- Диагностика встроена в рендеринг интерпретационных графиков.
...
## v222.13
Реализован многоинтервальный атлас отчёта: обзор скважины, детальные страницы групп пластов, адаптивный zoom и карточки интервалов.
...
## v222.16 — Project Explorer branch diagnostics

Статус: реализовано и проверено.

- Добавлены отдельные метрики чтения корневого проекта, карточек скважин, скважин, расчетов, экспортов, пользовательских папок и цветовых меток.
- Для каждого профиля раскрытых веток кэш публикует hits, misses, loads, hit rate и последнее время построения.
