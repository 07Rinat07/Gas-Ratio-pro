...
В режиме `Быстро: база знаний` помощник отвечает по локальным Q/A без ожидания модели.
Режим `Ollama: локальная модель` оставлен для более свободных ответов, но он может быть медленным.
...
4. Указать имя модели в локальном AI config.
...
В блоке `Чат поддержки` приложение показывает статус provider:
...
5. Укажите точное имя модели в локальном AI config.
