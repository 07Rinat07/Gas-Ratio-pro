...
## Безопасный локальный коммит в `main`

Для этапов Gas Ratio Pro рабочая ветка — только `main`. Чтобы не путаться между `main`, `master`, staging area и незакоммиченными изменениями, можно использовать helper-скрипт:

```powershell
.\scripts\commit_main.ps1 -Message "Move project explorer objects"
```

Скрипт переходит в корень репозитория, переключается на `main`, привязывает локальную ветку к `origin/main`, запускает `pytest` и `preflight`, затем выполняет `git add .` и локальный `git commit`. `git push` скрипт не делает: отправка остается ручной командой `git push origin main`.
