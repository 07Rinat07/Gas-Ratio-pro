...
def test_hydrocarbon_interval_engine_keeps_transition_candidates_when_enabled() -> None:
    ...
    assert result.schema.endswith("/v19")
...
def test_hydrocarbon_interval_engine_validation_case_passes_for_gas_reference() -> None:
    ...
    assert contract["schema"].endswith("/v19")
...
def test_hydrocarbon_interval_engine_exports_explanation_package() -> None:
    from core.hydrocarbon_intervals import build_hydrocarbon_interval_engine_payload

    frame = pd.DataFrame(
        {
            "depth": [3300.0, 3301.0, 3302.0],
            "interpretation": ["Газовая залежь", "Газовая залежь", "Газовая залежь"],
            "lithology": ["Sandstone", "Sandstone", "Sandstone"],
            "wh": [8.0, 8.5, 9.0],
            "bh": [44.0, 43.0, 42.0],
            "c1_c2": [82.0, 81.0, 80.0],
            "c1_c3": [180.0, 175.0, 170.0],
        }
    )
    ...
    assert result.schema.endswith("/v19")
    assert interval.explanation is not None
    assert interval.explanation.engineering_hypothesis is True
    assert interval.explanation.summary
    assert interval.explanation.reasoning
    assert interval.explanation.supporting_evidence
    assert interval.explanation.limitations
    assert interval.explanation.recommendations
    assert payload["intervals"][0]["explanation"]["summary"] == interval.explanation.summary
    assert payload["markers"][0]["explanation_summary"] == interval.explanation.summary
    assert payload["summary"]["main_intervals"][0]["explanation_summary"] == interval.explanation.summary


def test_hydrocarbon_interval_engine_exports_structured_recommendations_and_limitations() -> None:
    from core.hydrocarbon_intervals import (
    ...
    frame = pd.DataFrame(
        {
            "depth": [4100.0],
            "interpretation": ["Переходная зона / проверить"],
            "lithology": ["Unknown"],
            "wh": [None],
            "bh": [None],
            "c1_c2": [None],
    ...
    result = detect_hydrocarbon_intervals(frame, rules=HydrocarbonIntervalRuleSet(max_depth_gap=2.0))
    interval = result.intervals[0]
    limitations = build_interpretation_limitations(interval)
    recommendations = build_interpretation_recommendations(interval)
    payload = build_hydrocarbon_interval_engine_payload(result)

    assert result.schema.endswith("/v19")
    assert limitations
    assert recommendations
    assert all(item.category for item in limitations)
    assert all(item.severity for item in limitations)
    assert all(item.action for item in recommendations)
    assert interval.explanation is not None
    assert interval.explanation.structured_limitations == limitations
    assert interval.explanation.structured_recommendations == recommendations
    assert payload["intervals"][0]["explanation"]["structured_limitations"][0]["message"] == limitations[0].message
    assert payload["intervals"][0]["explanation"]["structured_recommendations"][0]["action"] == recommendations[0].action



def test_hydrocarbon_interval_engine_runs_builtin_validation_suite_v2() -> None:
    from core.hydrocarbon_intervals import (
    ...
    cases = hydrocarbon_validation_cases()
    results = run_hydrocarbon_validation_suite()
    catalog = hydrocarbon_validation_catalog_rows()

    assert VALIDATION_DATASET_VERSION.endswith('/v2')
    assert len(cases) >= 6
    assert len(results) == len(cases)
    assert all(result.passed for result in results)
    assert {row['case_id'] for row in catalog} == {case.case_id for case in cases}
    assert any(row['case_id'] == 'TC-004-CLAYSTONE-BARRIER' for row in catalog)


def test_hydrocarbon_interval_engine_validation_case_frame_is_public_and_deterministic() -> None:
    from core.hydrocarbon_intervals import hydrocarbon_validation_case_frame, detect_hydrocarbon_intervals

    frame = hydrocarbon_validation_case_frame('TC-001-DRY-GAS')
    result = detect_hydrocarbon_intervals(frame)

    assert list(frame['depth']) == [2800.0, 2801.0, 2802.0]
    assert result.intervals[0].fluid_type == 'gas'
    assert result.intervals[0].explanation is not None
    assert result.intervals[0].explanation.recommendations


def test_hydrocarbon_interpretation_engine_freeze_status_is_complete() -> None:
    from core.hydrocarbon_intervals import hydrocarbon_engine_freeze_status

    status = hydrocarbon_engine_freeze_status()

    assert status["version"] == "HIE v1.0"
    assert status["status"] == "frozen"
    assert status["validation_passed"] is True
    assert status["validation_case_count"] >= 6
    assert status["next_stage"] == "Professional Reporting System"
