...
def test_pdf_preview_ui_contains_opt_in_adjacent_range_prefetch() -> None:
    ...
    assert "application_service_container(export_state).pdf_preview(" in source
...
def test_pdf_preview_ui_exposes_memory_budget_and_eviction_telemetry() -> None:
    source = Path("app/streamlit_app.py").read_text(encoding="utf-8")
...
def test_pdf_preview_heavy_payload_is_kept_out_of_session_state() -> None:
    ...
    assert 'application_service_container(export_state).pdf_preview(' in source
    assert 'project_id=str(active_project.id)' in source
